...
describe("provisionPersonalTenant", () => {
  it("inserts org, membership, audit, and outbox for an existing user (no subscription — D-019)", async () => {
    ...
    // membership, audit, outbox. No subscription is granted at provisioning
    // (D-019 supersedes the trial-on-provisioning slice of D-015); a tenant
    // gets access only by explicitly claiming a plan later.
    ...
    expect(flat).not.toContain("subscriptions");
  });
