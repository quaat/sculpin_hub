...
`packages/db` owns persistence lifecycle. Applications receive a `Database` object from `createDatabase`/`getDatabase`; they must not create ad-hoc Prisma clients or pg pools. The lifecycle explicitly owns one bounded pg pool for readiness and transactional SQL plus one statically imported generated Prisma Client for generated schema access, and `close()` is idempotent. The pg pool defaults to `max=10`; the Prisma client factory injects `connection_limit=5` and a `connect_timeout` derived from the configured readiness timeout (rounded up to at least one second) into its PostgreSQL URL when the caller has not set those options. Each workload instance therefore budgets up to 15 PostgreSQL connections by default. This intentionally retains separate pools until a supported shared Prisma PostgreSQL adapter is adopted and tested. Readiness constructs Prisma once per `Database`, shares one `$connect()` promise across concurrent and repeated probes, retries a failed initial `$connect()` on the next readiness call, and always keeps the existing pg `SELECT 1` readiness query bounded by `query_timeout`.
