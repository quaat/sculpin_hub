...
3. **Proxy ↔ Sculpin upstream.** Server-side only. The Hub holds `OPENAI_DEV_API_KEY`; clients
   never see it or the internal URL.
...
| Upstream credential leakage | `OPENAI_DEV_API_KEY` never in DB/logs/usage/errors/responses; single injection module |
