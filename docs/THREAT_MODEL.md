...
| TEST-ONLY E2E auth seam reachable in prod | Seam disabled unless `E2E_TEST_AUTH="1"`; config HARD FAILS if enabled while `NODE_ENV=production`; the seam plugin is only wired in `buildE2EAuthOptions` (never in the production `buildAuthOptions`), which itself throws under production; the handler re-checks the flag and constant-time compares `x-e2e-seed-key`; it looks up an EXISTING seeded user only (never creates/provisions); the seed key is server-only and never enters a browser bundle (D-024) |
